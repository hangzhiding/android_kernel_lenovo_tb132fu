	...
	if (atomic_inc_return(&cs35l41_mclk_rsc_ref) == 1) {
		...
		/*2 slot config - bits 0 and 1 set for the first two slots
			...
			ret = snd_soc_dai_set_tdm_slot(codec_dais[i],
						...
						slots, slot_width);
			if (ret < 0) {
				dev_err(card->dev,"%s: cs35l43 failed to set tdm rx slot, err:%d\n",
				__func__, ret);
				return ret;
			} */
			...
			if (ret != 0) {
				dev_err(card->dev, "%s: cs35l43 Failed to set %s's fmt: ret = %d\n",
					codec_dais[i]->name, ret);
				return ret;
			}

			ret = snd_soc_component_set_sysclk(codec_dais[i]->component, 0, 0,clk_freq,SND_SOC_CLOCK_IN);
			if (ret < 0){
				dev_err(card->dev," %s: cs35l43 set sysclk failed, err:%d\n",codec_dais[i]->name, ret);
				return ret ;
			}	
		}
	}
