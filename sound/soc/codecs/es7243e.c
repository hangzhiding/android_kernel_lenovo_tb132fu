...
static int es7243e_probe(struct snd_soc_component *component)
{
	...
	for (index = 0; index < (ES7243E_CHANNELS_MAX) / 2; index++) {
		...
		es7243e_read(0xfd, &regv, i2c_clt[index]);
		if(0x72 == regv){
			printk("It is 7243l");
			//7243l
			es7243e_write(0x16, 0x00, i2c_clt[index]);
			es7243e_write(0x18, 0x24, i2c_clt[index]);
			es7243e_write(0x17, 0x02, i2c_clt[index]);
			es7243e_write(0x19, 0x88, i2c_clt[index]);
			es7243e_write(0x1a, 0x84, i2c_clt[index]);
			es7243e_write(0x1b, 0x66, i2c_clt[index]);
			es7243e_write(0x1c, 0x44, i2c_clt[index]);
			es7243e_write(0x1d, 0xf0, i2c_clt[index]);
	...
	if (of_get_property(i2c->dev.of_node, "vrf18_adc-supply", NULL)) {
		...
		if (regulator_count_voltages(es7243e->vdd) > 0) {
			ret = regulator_set_voltage(es7243e->vdd, MIN_VOL,
						    MAX_VOL);
			if (ret) {
				pr_err("vdd regulator set_vtg failed ret=%d", ret);
		...
		if (ret) {
			 pr_err("enable vdd regulator failed,ret=%d",ret);
		}
	} else {
		pr_err("vrf18_adc-supply no found in dts");
	}
#endif
