...
static kal_uint8 gc08a3_otp_2a_data[40] = {0};
static kal_uint8 gc08a3_otp_lsc_data[1868] = {0};
...
{
	LOG_INF("[%s] hs_video_Start, 1632x1224@30.19fps\n", __func__);
	table_write_cmos_sensor(gc08a3_1632x1224_addr_data,
		sizeof(gc08a3_1632x1224_addr_data)/sizeof(kal_uint16));
...
{
	LOG_INF("[%s] slim_video_Start, 1280x720@30.19fps\n", __func__);
...
{
	kal_uint8 flag, checksum_read;
	kal_uint16 i, addr;
	kal_uint16 group_2a[2]  = {0x15a0, 0x88e0};
	kal_uint16 group_lsc[2] = {0x1720, 0x8a60};
	kal_uint32 checksum_cal;

	LOG_INF("+");

	// read otp before sensor init
	write_cmos_sensor_8bit(0x031c, 0x60);
	write_cmos_sensor_8bit(0x0315, 0x80);

	// read otp initial setting
	...
	checksum_cal = 0;
	checksum_read = 0;
	memset(gc08a3_otp_2a_data, 0, sizeof(gc08a3_otp_2a_data));

	for (i = 0; i < 2; i++) {
		write_cmos_sensor_8bit(0x0313, 0x00);
		write_cmos_sensor_8bit(0x0a69, group_2a[i] >> 8);
		write_cmos_sensor_8bit(0x0a6a, group_2a[i] & 0xff);
	...
	LOG_INF("2a addr = 0x%04X", addr);

	if (addr) {
		for (i = 0; i < 40; i++) {
			gc08a3_otp_2a_data[i] = read_cmos_sensor(0x0a6c);
			LOG_INF("2a data [%d] = 0x%x", i, gc08a3_otp_2a_data[i]);
			checksum_cal += gc08a3_otp_2a_data[i];
		}

		checksum_read = read_cmos_sensor(0x0a6c);
		checksum_cal = checksum_cal % 255 + 1;

		if (checksum_read != checksum_cal) {
			LOG_INF("gc08a3 2a checksum err!");
			memset(gc08a3_otp_2a_data, 0, sizeof(gc08a3_otp_2a_data));
		}
	}
	...
	checksum_cal = 0;
	checksum_read = 0;
	memset(gc08a3_otp_lsc_data, 0, sizeof(gc08a3_otp_lsc_data));

	for (i = 0; i < 2; i++) {
		write_cmos_sensor_8bit(0x0313, 0x00);
		write_cmos_sensor_8bit(0x0a69, group_lsc[i] >> 8);
		write_cmos_sensor_8bit(0x0a6a, group_lsc[i] & 0xff);
		write_cmos_sensor_8bit(0x0313, 0x20);
		write_cmos_sensor_8bit(0x0313, 0x12);
	...
	LOG_INF("lsc addr = 0x%04X", addr);

	if (addr) {
		for (i = 0; i < 1868; i++) {
			gc08a3_otp_lsc_data[i] = read_cmos_sensor(0x0a6c);
			checksum_cal += gc08a3_otp_lsc_data[i];
		}
		...
		if (checksum_read != checksum_cal) {
			LOG_INF("gc08a3 lsc checksum err!");
			memset(gc08a3_otp_lsc_data, 0, sizeof(gc08a3_otp_lsc_data));
		}
	}
                                ...
                                unsigned char *data, unsigned int size)
{
	...
	if (size == 1) { // sensor id
		memcpy(data, &gc08a3_otp_2a_data[4], size);
		LOG_INF("sensor id = 0x%02X, 0x%02X", *data, gc08a3_otp_2a_data[4]);
	} else if (size == 40) {
		memcpy(data, &gc08a3_otp_2a_data[0], size);
	} else if (size == 1868) {
		memcpy(data, &gc08a3_otp_lsc_data[0], size);
	} else {
		ret = 0;
	}
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("[gc08a3_camera_sensor]get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				read_otp_data();
				return ERROR_NONE;
			}
			LOG_INF("[gc08a3_camera_sensor]get_imgsensor_id:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("[gc08a3_camera_sensor]open:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}
			LOG_INF("[gc08a3_camera_sensor]open:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
