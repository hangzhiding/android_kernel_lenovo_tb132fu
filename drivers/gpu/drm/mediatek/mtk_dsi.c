			...
			struct DSI_T0_INS *t0, int i, uintptr_t slot)
{
	...
	cmdq_pkt_write(handle, comp->cmdq_base, comp->regs_pa + DSI_CMDQ0,
		0x00013700, ~0);
	cmdq_pkt_write(handle, dsi->slave_dsi->ddp_comp.cmdq_base, dsi->slave_dsi->ddp_comp.regs_pa + DSI_CMDQ0,
		0x00013700, ~0);

	cmdq_pkt_write(handle, comp->cmdq_base, comp->regs_pa + DSI_CMDQ1,
		AS_UINT32(t0), ~0);
	cmdq_pkt_write(handle, dsi->slave_dsi->ddp_comp.cmdq_base, dsi->slave_dsi->ddp_comp.regs_pa + DSI_CMDQ1,
		AS_UINT32(t0), ~0);

	cmdq_pkt_write(handle, comp->cmdq_base, comp->regs_pa + DSI_CMDQ_SIZE,
		0x2, ~0);
	cmdq_pkt_write(handle, dsi->slave_dsi->ddp_comp.cmdq_base, dsi->slave_dsi->ddp_comp.regs_pa + DSI_CMDQ_SIZE,
		0x2, ~0);

	cmdq_pkt_write(handle, comp->cmdq_base, comp->regs_pa + DSI_START,
		0x0, ~0);
	cmdq_pkt_write(handle, dsi->slave_dsi->ddp_comp.cmdq_base, dsi->slave_dsi->ddp_comp.regs_pa + DSI_START,
		0x0, ~0);
	...
	mtk_dsi_cmdq_poll(comp, handle, comp->regs_pa + DSI_INTSTA, 0x1, 0x1);
	mtk_dsi_cmdq_poll(comp, handle, dsi->slave_dsi->ddp_comp.regs_pa + DSI_INTSTA, 0x1, 0x1);

	cmdq_pkt_write(handle, comp->cmdq_base, comp->regs_pa + DSI_INTSTA,
		0x0, 0x1);
	cmdq_pkt_write(handle, dsi->slave_dsi->ddp_comp.cmdq_base, dsi->slave_dsi->ddp_comp.regs_pa + DSI_INTSTA,
		0x0, 0x1);
	cmdq_pkt_mem_move(handle, comp->cmdq_base,comp->regs_pa + DSI_RX_DATA0, read_slot + (i * 2) * 0x4,
		CMDQ_THR_SPR_IDX3);
	cmdq_pkt_mem_move(handle, dsi->slave_dsi->ddp_comp.cmdq_base, dsi->slave_dsi->ddp_comp.regs_pa + DSI_RX_DATA0, read_slot + (i * 2 + 8) * 0x4,
		CMDQ_THR_SPR_IDX3);

	cmdq_pkt_mem_move(handle, comp->cmdq_base,comp->regs_pa + DSI_RX_DATA1, read_slot + (i * 2 + 1) * 0x4,
		CMDQ_THR_SPR_IDX3);
	cmdq_pkt_mem_move(handle, dsi->slave_dsi->ddp_comp.cmdq_base, dsi->slave_dsi->ddp_comp.regs_pa + DSI_RX_DATA1, read_slot + (i * 2 + 9) * 0x4,
		CMDQ_THR_SPR_IDX3);

	cmdq_pkt_write(handle, comp->cmdq_base, comp->regs_pa + DSI_RACK,
		0x1, 0x1);
	cmdq_pkt_write(handle, dsi->slave_dsi->ddp_comp.cmdq_base, dsi->slave_dsi->ddp_comp.regs_pa + DSI_RACK,
		0x1, 0x1);
...
int mtk_dsi_esd_cmp(struct mtk_ddp_comp *comp, void *handle, void *slot)
{
	...
	for (i = 0; i < ESD_CHECK_NUM; i++) {
		...
		pr_info("esd read value:reg:0x%x dsi0:0x%x, dsi1:0x%x\n", lcm_esd_tb->cmd, chk_val, chk_val1);
		if (chk_val == lcm_esd_tb->para_list[0] || chk_val1 == lcm_esd_tb->para_list[0]) {
			ret = 0;
		} else {
