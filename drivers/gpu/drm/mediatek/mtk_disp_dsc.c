...
void mtk_dsc_dump(struct mtk_ddp_comp *comp)
{
	...
	for (i = 0; i < 81; i += 16) {
		DDPDUMP("DSC+%x: 0x%x 0x%x 0x%x 0x%x\n", i + 0x100, readl(baddr + i + 0x100),
			 readl(baddr + i + 0x4 + 0x100), readl(baddr + i + 0x8 + 0x100),
			 readl(baddr + i + 0xc + 0x100));
	}
	for (i = 0; i < 204; i += 16) {
		DDPDUMP("DSC+%x: 0x%x 0x%x 0x%x 0x%x\n", i + 0x400, readl(baddr + i + 0x400),
			 readl(baddr + i + 0x4 + 0x400), readl(baddr + i + 0x8 + 0x400),
			 readl(baddr + i + 0xc + 0x400));
	}
	for (i = 0; i < 81; i += 16) {
		DDPDUMP("DSC+%x: 0x%x 0x%x 0x%x 0x%x\n", i + 0x500, readl(baddr + i + 0x500),
			 readl(baddr + i + 0x4 + 0x500), readl(baddr + i + 0x8 + 0x500),
			 readl(baddr + i + 0xc + 0x500));
	}
}
